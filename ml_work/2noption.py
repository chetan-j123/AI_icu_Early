...
if mode == "Live Simulation":
    ...
    with st.form("manual_entry"):
        ...
        if submit:
            # Construct Input
            input_dict = {
                'heart_rate': hr, 'spo2_pct': spo2, 'systolic_bp': sbp, 'diastolic_bp': dbp,
                'respiratory_rate': rr, 'temperature_c': temp, 'wbc_count': wbc, 'lactate': lactate,
                'oxygen_flow': 0, 'oxygen_device': 0, 'creatinine': 1.0, 'crp_level': 5.0, 
                'hemoglobin': 12.0, 'mobility_score': mobility,
                'nurse_alert': 1 if nurse_concern else 0, 'sepsis_risk_score': 0, 'age': age,
                'comorbidity_index': 1, 'hour_from_admission': 12, 'gender': 1, 
